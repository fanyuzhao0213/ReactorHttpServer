...
struct TcpConnection* tcpConnectionInit(int fd, struct EventLoop* evloop)  // TcpConnection初始化
{
    ...
    conn->evloop = evloop;
    conn->readBuf = bufferInit(10240);  //10K read buf
    conn->writeBuf = bufferInit(10240); //10K write buf
    sprintf(conn->name,"Connection-%d",fd);
    // http
    conn->request = httpRequestInit();
    conn->response = httpResponseInit();
    conn->channel = channelInit(fd,ReadEvent,processRead,processWrite,tcpConnectionDestroy,conn);//实例化channel
    eventLoopAddTask(evloop,conn->channel,ADD);//添加任务到任务队列 
...
int tcpConnectionDestroy(void* arg)// TcpConnection销毁
{
    struct TcpConnection* conn = (struct TcpConnection*)arg;
    if(conn != NULL)
    {
